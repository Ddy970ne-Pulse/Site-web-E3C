...
async def test_google_login_creates_new_user(client):
    await settings_store.update_settings(
        core.db, {"google_client_id": "test-google-client-id"}
    )
    fake_idinfo = {
    ...
    with patch.object(
        auth_routes.google_id_token, "verify_oauth2_token", return_value=fake_idinfo
    ):
        r = await client.post(
...
async def test_facebook_login_creates_new_user(client):
    ...
    fake_client = _FakeAsyncClient(
        {
            "debug_token": _FakeResponse(
                {"data": {"is_valid": True, "app_id": "fb-app-id"}}
            ),
            "/me": _FakeResponse(
                {"id": "fb-123", "name": "FB User", "email": "fbuser@example.com"}
            ),
        }
    )
...
def _apple_mocks(payload):
    ...
    return (
        patch(
            "routers.auth_routes._get_apple_jwks_client", return_value=fake_jwks_client
        ),
...
async def test_apple_login_creates_new_user(client):
    await settings_store.update_settings(
        core.db, {"apple_client_id": "apple-client-id"}
    )
    payload = {
    ...
    with p1, p2:
        r = await client.post(
            "/api/auth/apple", json={"id_token": "fake-jwt", "name": "Apple User"}
        )
    assert r.status_code == 200
...
async def test_auth_providers_reflects_configured_settings(client):
    await settings_store.update_settings(
        core.db, {"google_client_id": "gid", "facebook_app_id": "fid"}
    )
