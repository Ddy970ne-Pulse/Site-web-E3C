...
class TestTestimonials:
    ...
    def test_submit_testimonial_public(self):
        ...
        resp = requests.post(f"{BASE_URL}/api/testimonials", json={
    ...
    def test_approve_testimonial(self, admin_session):
        ...
        resp = admin_session.patch(f"{BASE_URL}/api/testimonials/{t_id}", json={"status": "approved"})
        assert resp.status_code == 200
        data = resp.json()
        assert "message" in data
    ...
    def test_reject_testimonial(self, admin_session):
        ...
        if not t_id:
            pytest.skip("No testimonial ID available")
        resp = admin_session.patch(f"{BASE_URL}/api/testimonials/{t_id}", json={"status": "rejected"})
        assert resp.status_code == 200
...
class TestGallery:
    ...
    def test_upload_gallery_requires_auth(self):
        ...
        tmp.close()
        resp = requests.post(f"{BASE_URL}/api/gallery",
    ...
    def test_upload_gallery_image(self, admin_session):
        ...
        import io
        resp = admin_session.post(f"{BASE_URL}/api/gallery",
