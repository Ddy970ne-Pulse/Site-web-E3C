...
async def test_paypal_create_order_success(client):
    ...
    with patch.object(
        payments_routes.paypal, "create_order", return_value=fake_order
    ) as mock_create:
        r = await client.post(
...
async def test_paypal_create_order_blocks_other_clients_invoice(client):
    ...
    other_token = core.create_access_token(
        other_uid, "someoneelse@example.com", "client"
    )
...
async def test_paypal_create_order_upstream_error_returns_502(client):
    ...
    with patch.object(
        payments_routes.paypal, "create_order", side_effect=httpx.HTTPError("boom")
    ):
        r = await client.post(
            "/api/payments/paypal/create-order",
            json={
...
async def test_paypal_capture_marks_tranche_paid(client):
    ...
    await core.db.payment_transactions.insert_one(
        {
            ...
            "tranche_id": tranche_id,
            "invoice_number": "FAC-2026-999",
            "tranche_label": "Acompte",
            "client_id": (await core.db.invoices.find_one({"id": invoice_id}))[
                "client_id"
            ],
            "client_name": "Payer",
            "amount": 100.0,
            "currency": "eur",
            "session_id": "PAYPAL-ORDER-1",
    ...
    with patch.object(
        payments_routes.paypal, "capture_order", return_value={"status": "COMPLETED"}
    ):
