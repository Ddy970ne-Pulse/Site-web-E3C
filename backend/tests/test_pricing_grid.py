...
def test_put_pricing_grid(admin_session, created_item_id):
    ...
    resp = admin_session.put(f"{BASE_URL}/api/pricing-grid/{created_item_id}", json=update)
...
def test_toggle_active_false(admin_session, created_item_id):
    resp = admin_session.put(f"{BASE_URL}/api/pricing-grid/{created_item_id}", json={"active": False})
    assert resp.status_code == 200
    data = resp.json()
...
def test_toggle_active_true(admin_session, created_item_id):
    resp = admin_session.put(f"{BASE_URL}/api/pricing-grid/{created_item_id}", json={"active": True})
    assert resp.status_code == 200
    data = resp.json()
...
def test_delete_pricing_grid(admin_session):
    ...
    payload = {"category": "TEST_Delete", "description": "TEST_To be deleted", "unit": "u", "unit_price_ht": 10.0, "tva_rate": 20.0, "active": True}
    resp = admin_session.post(f"{BASE_URL}/api/pricing-grid", json=payload)
    assert resp.status_code == 200
    item_id = resp.json()["id"]
...
def test_post_requires_admin(admin_session):
    ...
    payload = {"category": "Test", "description": "Test", "unit": "u", "unit_price_ht": 10.0, "tva_rate": 20.0}
