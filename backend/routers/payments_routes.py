...
async def create_checkout(body: CheckoutRequest, request: Request):
    ...
    tranche = next(
        (t for t in inv.get("payment_tranches", []) if t["id"] == body.tranche_id), None
    )
    if not tranche:
    ...
    if existing_tx:
        return {
            "checkout_url": existing_tx.get("checkout_url", ""),
            "session_id": existing_tx["session_id"],
        }
        ...
        line_items=[
            {
                "price_data": {
                    ...
                    "product_data": {
                        "name": f"{inv['invoice_number']} — {tranche['label']}"
                    },
                },
                "quantity": 1,
            }
...
async def capture_paypal_order(order_id: str, request: Request):
    ...
    if result.get("status") == "COMPLETED":
        ...
        await _mark_tranche_paid(
            tx["invoice_id"], tx["tranche_id"], payment_method="paypal"
        )
        return {"payment_status": "paid"}
...
async def list_payments(request: Request):
    ...
    txs = (
        await db.payment_transactions.find(query, {"_id": 0})
        .sort("created_at", -1)
        .to_list(500)
    )
    return txs
...
async def mark_tranche_paid_manually(
    invoice_id: str, tranche_id: str, body: MarkTranchePaidRequest, request: Request
):
    ...
    if not inv:
        raise HTTPException(404, "Facture introuvable")
    tranche = next(
        (t for t in inv.get("payment_tranches", []) if t["id"] == tranche_id), None
    )
    if not tranche:
        raise HTTPException(404, "Tranche introuvable")
    if tranche["status"] == "paid":
...
async def stripe_webhook(request: Request):
    ...
    if not webhook_secret:
        logger.error(
            "STRIPE_WEBHOOK_SECRET non configuré — webhook ignoré (payload non vérifié)"
        )
...
async def _mark_tranche_paid(
    invoice_id: str, tranche_id: str, payment_method: str = "carte"
):
    inv = await db.invoices.find_one({"id": invoice_id}, {"_id": 0})
    if not inv:
        return
    ...
    if all_paid:
        ...
        await send_email(
            ADMIN_EMAIL, f"Paiement complet - Facture {inv['invoice_number']}", html
        )
