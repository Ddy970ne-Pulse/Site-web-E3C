...
from core import (
    AUTH_RATE_LIMIT, create_access_token, create_refresh_token, db,
    get_current_user, hash_password, limiter, set_auth_cookies, verify_password,
)
...
@limiter.limit(AUTH_RATE_LIMIT)
async def facebook_auth(request: Request, body: FacebookAuthRequest, response: Response):
    settings = await settings_store.get_settings(db)
    facebook_app_id = settings.get("facebook_app_id", "")
