...
export default function Footer({ whatsapp }) {
  ...
  return (
    <footer
      data-testid="footer"
      className="bg-[#050505] border-t border-white/5 pt-16 md:pt-20 pb-8"
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 mb-14">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="space-y-2">
              <Link to="/connexion" className="flex items-center gap-2 text-gray-400 hover:text-[#D4AF37] text-xs transition-colors">
                <LogIn size={12} /> Se connecter à l'espace client
              </Link>
              <Link to="/inscription" className="flex items-center gap-2 text-gray-400 hover:text-[#D4AF37] text-xs transition-colors">
          ...
          {/* Navigation */}
          <div>
            <h4 className="font-outfit font-semibold text-white text-sm uppercase tracking-widest mb-4">
              Navigation
            </h4>
            <ul className="space-y-2.5">
              {[
                ...
                <li key={item.label}>
                  {item.to ? (
                    <Link to={item.to} className="text-gray-400 hover:text-[#D4AF37] text-sm transition-colors">{item.label}</Link>
                  ) : (
                    <button onClick={() => scrollTo(item.scroll)} className="text-gray-400 hover:text-[#D4AF37] text-sm transition-colors">{item.label}</button>
                  )}
                </li>
              ))}
              <li>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {services.map((s) => (
                <li key={s} className="text-gray-500 text-xs">{s}</li>
              ))}
            </ul>
          </div>
