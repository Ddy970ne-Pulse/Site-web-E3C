  ...
  return (
    <div className="bg-[#121212] border border-[#D4AF37]/20 rounded-sm p-6 mb-6" data-testid="quote-form">
      <div className="flex items-center justify-between mb-5">
        <h2 className="font-outfit font-bold text-white text-lg">{quote ? "Modifier le devis" : "Nouveau devis"}</h2>
      ...
      <form onSubmit={submit} className="space-y-5">
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs text-gray-400 uppercase tracking-wider mb-2">Valable jusqu'au *</label>
            <input type="date" value={form.valid_until} onChange={e => setField("valid_until", e.target.value)} required
              ...
              className="w-full bg-white/5 border border-white/10 text-white px-4 py-3 text-sm rounded-sm" />
          </div>
        </div>
        <div>
          <label className="block text-xs text-gray-400 uppercase tracking-wider mb-2">Description du projet *</label>
        ...
        <div>
          <label className="block text-xs text-gray-400 uppercase tracking-wider mb-2">Adresse du chantier</label>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <label className="text-xs text-gray-400 uppercase tracking-wider">Lignes de devis</label>
            <button type="button" onClick={addItem} className="flex items-center gap-1 text-[#D4AF37] text-xs font-semibold hover:underline">
              ...
              <div key={i} className="grid grid-cols-12 gap-2 items-center bg-white/3 border border-white/5 p-3 rounded-sm">
                <div className="col-span-12 md:col-span-5">
                  <input placeholder="Description de la prestation" value={item.description}
                    ...
                    className="w-full bg-white/5 border border-white/10 text-white placeholder-gray-500 px-3 py-2 text-xs rounded-sm" />
                </div>
                <div className="col-span-4 md:col-span-2">
                  <input type="number" placeholder="Qté" value={item.quantity} min="0" step="0.1"
                    onChange={e => setItem(i, "quantity", e.target.value)}
                    className="w-full bg-white/5 border border-white/10 text-white px-3 py-2 text-xs rounded-sm" />
                </div>
                <div className="col-span-4 md:col-span-2">
          ...
          <div className="mt-4 text-right space-y-1">
            <p className="text-gray-400 text-sm">Total HT : <span className="text-white font-medium">{totalHT.toFixed(2)} €</span></p>
            <p className="text-gray-400 text-sm">TVA : <span className="text-white font-medium">{totalTVA.toFixed(2)} €</span></p>
            <p className="text-[#D4AF37] font-bold text-lg">TOTAL TTC : {totalTTC.toFixed(2)} €</p>
          </div>
        </div>

        <div>
          <label className="block text-xs text-gray-400 uppercase tracking-wider mb-2">Notes / Conditions</label>
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={loading} data-testid="quote-save-btn"
            className="flex items-center gap-2 bg-[#D4AF37] text-black font-bold px-6 py-3 hover:bg-[#E6C65A] transition-colors disabled:opacity-60 text-sm">
            <Save size={15}/> {loading ? "Sauvegarde..." : "Sauvegarder"}
          </button>
          <button type="button" onClick={onCancel} className="px-6 py-3 border border-white/20 text-gray-300 hover:border-white/40 text-sm transition-colors">
            Annuler
          </button>
        </div>
      </form>
