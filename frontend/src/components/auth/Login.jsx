...
export default function Login() {
  const { login, googleLogin, facebookLogin, appleLogin, providers } = useAuth();
    ...
    <div className="min-h-screen bg-[#FAFAF8] dark:bg-[#0A0A0A] flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <h1 className="font-outfit font-bold text-3xl text-[#1A1A1A] dark:text-white">Connexion</h1>
          <p className="text-[#737373] dark:text-gray-400 text-base mt-1">Accédez à votre espace personnel</p>
        </div>
        ...
        <div className="bg-white dark:bg-[#121212] border border-black/8 dark:border-white/8 p-8 rounded-sm shadow-sm dark:shadow-none">
          {error && (
            <div data-testid="login-error" className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-600 dark:text-red-400 p-3 mb-5 text-base rounded-sm">
              {error}
            </div>
          )}
          {socialAuthEnabled && (
          ...
          <form onSubmit={submit} className="space-y-5">
            ...
            <button type="submit" disabled={loading} data-testid="login-submit"
              className="w-full flex items-center justify-center gap-2 bg-[#D4AF37] text-black font-bold py-3.5 hover:bg-[#E6C65A] transition-colors disabled:opacity-60 text-base tracking-wide rounded-sm">
          ...
          <div className="mt-6 text-center border-t border-black/7 dark:border-white/5 pt-5">
            <p className="text-[#737373] dark:text-gray-400 text-base">Pas encore de compte ?{" "}
              <Link to="/inscription" className="text-[#D4AF37] hover:underline font-medium">Créer un compte</Link>
            </p>
          </div>
        </div>
        <div className="mt-4 text-center">
          <Link to="/" className="text-[#9E9E9E] dark:text-gray-500 text-sm hover:text-[#4B4B4B] dark:hover:text-gray-300 transition-colors">
            ← Retour au site
          </Link>
        </div>
      </div>
