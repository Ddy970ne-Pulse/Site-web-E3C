  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#121212] border border-white/8 p-8">
          {error && (
            <div data-testid="register-error" className="bg-red-500/10 border border-red-500/30 text-red-400 p-3 mb-5 text-sm rounded-sm">
              {error}
            </div>
          )}
          <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs text-gray-400 uppercase tracking-wider mb-2">Téléphone</label>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPw ? "text" : "password"} name="password" value={form.password} onChange={handle} required minLength={6}
                  ...
                  className="w-full bg-white/5 border border-white/10 text-white placeholder-gray-500 px-4 py-3 pr-10 text-sm rounded-sm"
        ...
        <div className="mt-4 text-center">
          <Link to="/" className="text-gray-500 text-xs hover:text-gray-300 transition-colors">← Retour au site</Link>
        </div>
      </div>
    </div>
  );
