      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          <Link to="/" className="inline-flex items-center gap-3 mb-6">
            ...
            <span className="font-outfit font-bold text-[#1A1A1A] dark:text-white text-2xl">E3C — Entreprise de constructions</span>
          </Link>
          <h1 className="font-outfit font-bold text-3xl text-[#1A1A1A] dark:text-white">Créer un compte</h1>
          <p className="text-[#737373] dark:text-gray-400 text-base mt-1">Accédez à vos devis et factures</p>
        </div>
        ...
        <div className="bg-white dark:bg-[#121212] border border-black/8 dark:border-white/8 p-8 rounded-sm shadow-sm dark:shadow-none">
          {error && (
            <div data-testid="register-error" className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-600 dark:text-red-400 p-3 mb-5 text-base rounded-sm">
              {error}
            </div>
          )}
          {socialAuthEnabled && (
            <>
              ...
              <div className="flex items-center gap-3 my-6">
                ...
                <span className="text-sm text-[#9E9E9E] dark:text-gray-500 uppercase tracking-wider">ou</span>
                <div className="flex-1 h-px bg-black/10 dark:bg-white/10" />
              </div>
            </>
          )}
          <form onSubmit={submit} className="space-y-4">
            <div>
              ...
              <input type="text" name="name" value={form.name} onChange={handle} required
                ...
                className="w-full bg-[#F5F4F0] dark:bg-white/5 border border-black/10 dark:border-white/10 text-[#1A1A1A] dark:text-white placeholder-[#ADADAD] dark:placeholder-gray-500 px-4 py-3 text-base rounded-sm"
            ...
            <div>
              <label className="block text-sm text-[#737373] dark:text-gray-400 uppercase tracking-wider mb-2">Téléphone</label>
            ...
            <div>
              <label className="block text-sm text-[#737373] dark:text-gray-400 uppercase tracking-wider mb-2">Mot de passe *</label>
              <div className="relative">
                <input type={showPw ? "text" : "password"} name="password" value={form.password} onChange={handle} required minLength={6}
                  data-testid="register-password"
                  className="w-full bg-[#F5F4F0] dark:bg-white/5 border border-black/10 dark:border-white/10 text-[#1A1A1A] dark:text-white placeholder-[#ADADAD] dark:placeholder-gray-500 px-4 py-3 pr-10 text-base rounded-sm"
            ...
            <button type="submit" disabled={loading} data-testid="register-submit"
              className="w-full flex items-center justify-center gap-2 bg-[#D4AF37] text-black font-bold py-3.5 hover:bg-[#E6C65A] transition-colors disabled:opacity-60 text-base tracking-wide rounded-sm mt-2">
          ...
          <div className="mt-5 text-center border-t border-black/7 dark:border-white/5 pt-4">
            <p className="text-[#737373] dark:text-gray-400 text-base">Déjà un compte ?{" "}
              <Link to="/connexion" className="text-[#D4AF37] hover:underline font-medium">Se connecter</Link>
            </p>
          </div>
        </div>
        <div className="mt-4 text-center">
          <Link to="/" className="text-[#9E9E9E] dark:text-gray-500 text-sm hover:text-[#4B4B4B] dark:hover:text-gray-300 transition-colors">← Retour au site</Link>
        </div>
      </div>
    </div>
  );
