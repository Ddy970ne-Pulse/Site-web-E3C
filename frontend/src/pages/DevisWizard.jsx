...
function StepIndicator({ current, total }) {
  return (
    <div className="flex items-center gap-2 mb-8">
      ...
      <span className="ml-2 text-xs text-[#9E9E9E] dark:text-gray-500">{current + 1} / {total}</span>
    </div>
  );
}
      ...
      <div className="bg-[#FAFAF8] dark:bg-[#0A0A0A] min-h-screen">
        ...
        <div className="min-h-screen flex items-center justify-center px-6 pt-20">
          <div className="max-w-lg w-full text-center">
            ...
            <h1 className="font-outfit font-bold text-3xl text-[#1A1A1A] dark:text-white mb-3">Demande envoyée !</h1>
            <p className="text-[#737373] dark:text-gray-400 text-base mb-6 leading-relaxed">
            ...
            <div className="bg-white dark:bg-[#121212] border border-black/7 dark:border-white/5 p-5 rounded-sm mb-6 text-left">
              <p className="text-xs text-[#9E9E9E] dark:text-gray-500 uppercase tracking-widest mb-3 font-semibold">Récapitulatif</p>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button onClick={() => navigate(user ? "/espace-client" : "/")}
                className="flex-1 border border-black/15 dark:border-white/20 text-[#4B4B4B] dark:text-gray-300 font-semibold py-3 hover:border-black/30 dark:hover:border-white/40 hover:text-[#1A1A1A] dark:hover:text-white transition-colors text-sm rounded-sm">
    ...
    <div className="bg-[#FAFAF8] dark:bg-[#0A0A0A] min-h-screen">
      ...
      <div className="max-w-2xl mx-auto px-5 pt-28 pb-16">
        ...
        <div className="mb-8">
          ...
          <h1 className="font-outfit font-bold text-2xl sm:text-3xl text-[#1A1A1A] dark:text-white">Construisez votre demande</h1>
          <p className="text-[#737373] dark:text-gray-400 text-sm mt-1">Quelques questions pour préparer votre devis personnalisé</p>
        </div>
        ...
        <div className="bg-white dark:bg-[#121212] border border-black/7 dark:border-white/5 rounded-sm p-6 md:p-8">
          ...
          {step === 0 && (
            <div>
              <h2 className="font-outfit font-bold text-[#1A1A1A] dark:text-white text-xl mb-1">Quel type de projet ?</h2>
              <p className="text-[#737373] dark:text-gray-400 text-sm mb-6">Sélectionnez la catégorie qui correspond à vos travaux</p>
                ...
                {PROJECT_TYPES.map(pt => {
                  ...
                  return (
                    <button key={pt.id} onClick={() => set("project_type", pt.id)}
                      ...
                      <div>
                        <p className={`font-semibold text-sm ${data.project_type === pt.id ? "text-[#D4AF37]" : "text-[#1A1A1A] dark:text-white"}`}>{pt.label}</p>
                        <p className="text-[#9E9E9E] dark:text-gray-500 text-xs mt-0.5">{pt.desc}</p>
                      </div>
          ...
          {step === 1 && (
            <div>
              <h2 className="font-outfit font-bold text-[#1A1A1A] dark:text-white text-xl mb-1">Quels corps de métier ?</h2>
              <p className="text-[#737373] dark:text-gray-400 text-sm mb-6">Sélectionnez une ou plusieurs prestations <span className="text-[#D4AF37]">({data.services.length} sélectionnée{data.services.length > 1 ? "s" : ""})</span></p>
                ...
                {SERVICES_LIST.map(s => {
                  ...
                  return (
                    <button key={s.id} onClick={() => toggleService(s.id)}
                      ...
                      <span className={`text-sm font-medium ${selected ? "text-[#D4AF37]" : "text-[#4B4B4B] dark:text-gray-300"}`}>{s.label}</span>
                    </button>
                  );
                })}
              </div>
          ...
          {step === 2 && (
            <div>
              <h2 className="font-outfit font-bold text-[#1A1A1A] dark:text-white text-xl mb-1">Estimation tarifaire</h2>
              <p className="text-[#737373] dark:text-gray-400 text-sm mb-1">
              ...
              {pricingItems.length === 0 ? (
                <div className="border border-dashed border-black/10 dark:border-white/10 rounded-sm p-8 text-center">
                  <p className="text-[#9E9E9E] dark:text-gray-500 text-sm">La grille tarifaire n'est pas encore disponible.</p>
                  <p className="text-[#ADADAD] dark:text-gray-600 text-xs mt-1">Passez à l'étape suivante pour soumettre votre demande.</p>
                </div>
              ) : (
                <div className="space-y-6">
                          ...
                          return (
                            <div key={item.id} className={`flex items-center gap-3 p-3 border rounded-sm transition-colors ${qty ? "border-[#D4AF37]/30 bg-[#D4AF37]/5" : "border-black/5 dark:border-white/5 bg-black/2 dark:bg-white/2"}`}>
                              <div className="flex-1 min-w-0">
                                <p className="text-[#1A1A1A] dark:text-white text-sm font-medium leading-snug">{item.description}</p>
                                <p className="text-[#9E9E9E] dark:text-gray-500 text-xs">{item.unit_price_ht.toFixed(2)} € HT / {item.unit} · TVA {item.tva_rate}%</p>
                              </div>
                              <div className="flex items-center gap-2 flex-shrink-0">
                                <input
                                  ...
                                  className="w-20 bg-[#F5F4F0] dark:bg-[#0A0A0A] border border-black/10 dark:border-white/15 text-[#1A1A1A] dark:text-white text-sm px-2 py-1.5 rounded-sm text-right focus:outline-none focus:border-[#D4AF37]/50"
                                />
                                <span className="text-[#9E9E9E] dark:text-gray-500 text-xs w-8">{item.unit}</span>
                  ...
                  {otherItems.length > 0 && (
                    <details className="group">
                      <summary className="text-xs text-[#9E9E9E] dark:text-gray-500 cursor-pointer hover:text-[#4B4B4B] dark:hover:text-gray-300 transition-colors list-none flex items-center gap-2">
                      ...
                      <div className="mt-3 space-y-2">
                        {otherItems.map(item => {
                          ...
                          return (
                            <div key={item.id} className={`flex items-center gap-3 p-3 border rounded-sm transition-colors ${qty ? "border-[#D4AF37]/30 bg-[#D4AF37]/5" : "border-black/5 dark:border-white/5 bg-black/2 dark:bg-white/2"}`}>
                              <div className="flex-1 min-w-0">
                                <p className="text-[#4B4B4B] dark:text-gray-300 text-sm">{item.description}</p>
                                <p className="text-[#9E9E9E] dark:text-gray-500 text-xs">{item.category} · {item.unit_price_ht.toFixed(2)} € HT / {item.unit}</p>
                              </div>
                              <div className="flex items-center gap-2 flex-shrink-0">
                                <input type="number" min="0" step="0.1" value={qty} onChange={e => setQty(item.id, e.target.value)}
                                  placeholder="0" className="w-20 bg-[#F5F4F0] dark:bg-[#0A0A0A] border border-black/10 dark:border-white/15 text-[#1A1A1A] dark:text-white text-sm px-2 py-1.5 rounded-sm text-right focus:outline-none focus:border-[#D4AF37]/50" />
                                <span className="text-[#9E9E9E] dark:text-gray-500 text-xs w-8">{item.unit}</span>
                                {qty > 0 && <span className="text-[#D4AF37] text-xs font-semibold w-20 text-right">{lineTotal.toFixed(2)} €</span>}
                              </div>
                            </div>
                  ...
                  {lineItems.length > 0 && (
                    <div className="bg-[#F5F4F0] dark:bg-[#0D0D0D] border border-[#D4AF37]/20 rounded-sm p-4">
                      <p className="text-xs text-[#9E9E9E] dark:text-gray-500 uppercase tracking-widest mb-3 font-semibold">Récapitulatif estimation</p>
                      ...
                      <div className="border-t border-black/7 dark:border-white/5 pt-3 space-y-1">
                        <div className="flex justify-between text-sm"><span className="text-[#9E9E9E] dark:text-gray-500">Total HT</span><span className="text-[#4B4B4B] dark:text-gray-300">{totalHT.toFixed(2)} €</span></div>
          ...
          {step === 3 && (
            <div>
              <h2 className="font-outfit font-bold text-[#1A1A1A] dark:text-white text-xl mb-1">Décrivez votre projet</h2>
              <p className="text-[#737373] dark:text-gray-400 text-sm mb-6">Plus vous êtes précis, plus notre devis sera adapté</p>
              <div className="space-y-4">
                <div>
                  <label className="block text-xs text-[#737373] dark:text-gray-400 uppercase tracking-wider mb-2">Description détaillée *</label>
                  <textarea
                    ...
                    className="w-full bg-[#F5F4F0] dark:bg-white/5 border border-black/10 dark:border-white/10 text-[#1A1A1A] dark:text-white placeholder-[#ADADAD] dark:placeholder-gray-600 px-4 py-3 text-sm rounded-sm resize-none"
                  />
                  <p className={`text-xs mt-1 ${data.description.length < 10 ? "text-[#9E9E9E] dark:text-gray-500" : "text-green-600 dark:text-green-400"}`}>
                    {data.description.length} caractères {data.description.length < 10 ? "(minimum 10)" : "✓"}
                  </p>
                </div>
                <div>
                  <label className="block text-xs text-[#737373] dark:text-gray-400 uppercase tracking-wider mb-2">Surface approximative</label>
          ...
          {step === 4 && (
            <div>
              <h2 className="font-outfit font-bold text-[#1A1A1A] dark:text-white text-xl mb-1">Quelle commune ?</h2>
              <p className="text-[#737373] dark:text-gray-400 text-sm mb-6">Indiquez votre commune pour que nous puissions planifier notre déplacement</p>
              <div>
                <label className="block text-xs text-[#737373] dark:text-gray-400 uppercase tracking-wider mb-2">Commune *</label>
                <select
          ...
          {step === 5 && (
            <div>
              <h2 className="font-outfit font-bold text-[#1A1A1A] dark:text-white text-xl mb-1">Budget & Délai souhaités</h2>
              <p className="text-[#737373] dark:text-gray-400 text-sm mb-6">Ces informations sont indicatives et sans engagement</p>
              <div className="space-y-6">
                <div>
                  <label className="block text-xs text-[#737373] dark:text-gray-400 uppercase tracking-wider mb-3">Budget estimatif</label>
                ...
                <div>
                  <label className="block text-xs text-[#737373] dark:text-gray-400 uppercase tracking-wider mb-3">Délai souhaité</label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {DELAYS.map(d => (
          ...
          {step === 6 && (
            <div>
              <h2 className="font-outfit font-bold text-[#1A1A1A] dark:text-white text-xl mb-1">Vos coordonnées</h2>
              <p className="text-[#737373] dark:text-gray-400 text-sm mb-6">Pour vous envoyer votre devis personnalisé</p>
              {user && (
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs text-[#737373] dark:text-gray-400 uppercase tracking-wider mb-2">Téléphone (optionnel)</label>
                ...
                {!user && (
                  <div className="bg-[#D4AF37]/5 border border-[#D4AF37]/15 p-4 rounded-sm">
                    <p className="text-[#1A1A1A] dark:text-white text-sm font-semibold mb-1">Créez un compte pour suivre votre devis</p>
                    <p className="text-[#737373] dark:text-gray-400 text-xs mb-3">Accédez à votre espace personnel pour consulter et signer votre devis en ligne.</p>
                    <button onClick={() => navigate("/inscription")}
              ...
              {/* Summary */}
              <div className="mt-6 p-4 bg-black/3 dark:bg-white/3 border border-black/5 dark:border-white/5 rounded-sm space-y-2 text-sm">
                <p className="text-xs text-[#9E9E9E] dark:text-gray-500 uppercase tracking-wider font-semibold mb-3">Récapitulatif de votre demande</p>
        ...
        <div className="flex items-center justify-between mt-5">
          <button onClick={() => step > 0 ? setStep(s => s - 1) : navigate(-1)}
            className="flex items-center gap-2 text-[#737373] dark:text-gray-400 hover:text-[#1A1A1A] dark:hover:text-white transition-colors text-sm font-medium px-4 py-2.5 rounded-sm hover:bg-black/5 dark:hover:bg-white/5">
